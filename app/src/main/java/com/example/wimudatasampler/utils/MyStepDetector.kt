...
class MyStepDetector : SensorEventListener {
    ...
    //用于存放计算阈值的波峰波谷差值
    var tempValue: FloatArray = FloatArray(ValueNum)
    var tempCount: Int = 0

    //是否上升的标志位
    var isDirectionUp: Boolean = false

    //持续上升次数
    var continueUpCount: Int = 0

    //上一点的持续上升的次数，为了记录波峰的上升次数
    var continueUpFormerCount: Int = 0

    //上一点的状态，上升还是下降
    var lastStatus: Boolean = false

    //波峰值
    var peakOfWave: Float = 0f

    //波谷值
    var valleyOfWave: Float = 0f
    ...
    //当前传感器的值
    var gravityNew: Float = 0f

    //上次传感器的值
    var gravityOld: Float = 0f

    //动态阈值需要动态的数据，这个值用于这些动态数据的阈值
    val InitialValue: Float = 1.3.toFloat()

    //初始阈值
    var ThreadValue: Float = 2.0.toFloat()

    //波峰波谷时间差
    var TimeInterval: Int = 250
    private var mStepListeners: SensorUtils.SensorDataListener? = null
    ...
    override fun onSensorChanged(event: SensorEvent) {
        ...
        gravityNew =
            sqrt((oriValues[0] * oriValues[0] + oriValues[1] * oriValues[1] + oriValues[2] * oriValues[2]).toDouble()).toFloat()
    ...
    fun registerListener(listener: SensorUtils.SensorDataListener) {
